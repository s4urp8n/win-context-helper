...
**Flatten folder** — Each selected folder is flattened independently; a selected folder inside another selected folder, or one selected twice, is flattened once as part of the outer folder (folders are matched by their identity on disk, so letter case, junctions and short 8.3 names do not matter, while `Straße` and `Strasse` stay two folders). Files are taken in Explorer order — subfolders before files, case ignored, numbers compared by value (`Lesson 2` before `Lesson 10`), hyphens and apostrophes ignored — so on a name collision the first file keeps its name and later ones get ` (2)`, ` (3)`… Names that differ only in letter case count as the same name, as on NTFS. Every subfolder left empty is removed, including subfolders that were empty before; a folder with nothing but empty subfolders is not "already flat". Links (junctions, symlinks) are neither followed nor moved, so a subfolder holding one stays, and the dialog says how many such subfolders remain. The confirm dialog names the folder, shows the whole plan as a table (from → new name, one group per selected folder when several are selected), marks shortened and suffixed names, lists up to 1000 rows and warns that the operation cannot be cancelled; Enter there means Cancel. While files are moved the window cannot be closed.

Names are checked before anything moves: a name may have at most 255 characters and the full path at most 259 (so Explorer and players can still open the file). A name that does not fit is shortened with `…`, keeping the extension (only a short tail without spaces counts as one: `1. Introduction` has none); if even that is impossible because the folder path itself is too long, the dialog refuses to start and says which files are affected. The move is all-or-nothing: if any file cannot be moved (open in another program, access denied, a file with that name appeared), every file moved so far is put back, empty subfolders stay, and the error dialog says the folders are unchanged. A file that cannot be put back is listed with its current name in the root, and the list also goes to the log. Existing files are never overwritten: the target name is checked right before each move, and anything there — even a broken link — counts as taken. The run carries out exactly the plan shown in the preview: if the folder changed while the dialog was open, nothing is moved and the dialog asks to run the command again.

**Flatten folder (keep order)** — Like Flatten folder, but every file from a subfolder gets its full folder path in the name, joined with ` - ` at any depth: `Part A\Lesson 1\Chapter 1.mp4` → `Part A - Lesson 1 - Chapter 1.mp4`. Sorting the result by name keeps lessons and chapters in their original order, which plain flattening would mix up. Files already in the root keep their names; a name that is still taken gets ` (N)`. When a joined name is too long, the file's own name is cut first (down to 40 characters), then folder names; a folder is cut the same way for every file inside it, so its files keep a common head. If the shortened names would no longer sort like the full ones — or an `…` next to a punctuation mark decides their order, where Explorer and the check might disagree — every moved file of that folder gets a number prefix in the full-name order: `001 - Lesson 1 - Intro… - Chapter 1.mp4`. Files that already were in the root keep their names.
