...
Windows utility that adds a **ContextHelper** submenu to the Explorer right-click menu with 11 file and folder tools: flatten/merge/clean folders, archive, convert images and audio, build/merge/split PDFs, extract audio, join videos.

Per-user installer and registration (HKCU), no admin rights. Built on Electron; heavy lifting is done by bundled 7-Zip, ImageMagick, Ghostscript and ffmpeg.
...
| **Flatten folder** | 1+ folders | confirm with options | All nested files moved into the root of each folder, empty subfolders removed; by default each file is named after its folder path: `Lesson 1\Chapter 1.mp4` → `Lesson 1 - Chapter 1.mp4` |
...
**Flatten folder** — Each selected folder is flattened independently; a selected folder inside another selected folder, or one selected twice, is flattened once as part of the outer folder (folders are matched by their identity on disk, so letter case, junctions and short 8.3 names do not matter, while `Straße` and `Strasse` stay two folders). The confirm dialog has two options, both on by default; changing one rebuilds the preview at once, without a new scan:

- **Keep hierarchy — put folder names into file names.** Every file from a subfolder gets its folder path in the name, joined with ` - ` at any depth: `Part A\Lesson 1\Chapter 1.mp4` → `Part A - Lesson 1 - Chapter 1.mp4`, so sorting the result by name keeps the original order. Turned off, files keep their own names.
- **Folders before files, as Explorer lists them** (only with the hierarchy). On, the original order is the one Explorer shows: on every level the subfolders come before the files. Off, folders and files of a level are ordered together by name.

Names are compared exactly as Explorer compares them: the plugin calls Windows' own `StrCmpLogicalW` (through [koffi](https://koffi.dev/)), so case is ignored, numbers are compared by value (`Lesson 2` before `Lesson 10`) and punctuation sorts the Windows way (`X.mp4` before `X_en.vtt`). If the function cannot be loaded, a close ICU approximation is used. Before anything moves, the plugin checks that the new names, sorted like Explorer sorts them, list the files in the original order. When they do not — files lying in the root next to subfolders, a subfolder among the files of a level, names cut to fit — every file of that folder, including the files already in its root, gets a number prefix in the original order: `001 - Lesson 1 - Chapter 1.mp4` … `075 - Intro.mp4`. Otherwise the names carry no numbers and root files keep their names. A folder with nothing to move is left alone: its root files are never renumbered.

Without the hierarchy, files are taken in Explorer order (subfolders before files), so on a name collision the first file keeps its name and later ones get ` (2)`, ` (3)`… With the hierarchy, a name that is still taken gets ` (N)` as well. Names that differ only in letter case count as the same name, as on NTFS. Every subfolder left empty is removed, including subfolders that were empty before; a folder with nothing but empty subfolders is not "already flat". Links (junctions, symlinks) are neither followed nor moved, so a subfolder holding one stays, and the dialog says how many such subfolders remain. The confirm dialog has three titled blocks: **Options**; **Summary**, one short line per fact (folder, files to move, subfolders removed or kept, numbering, shortened and suffixed names, what happens on error, skipped items); and **Preview**, the whole plan as a table (from → new name, under a bold header naming each folder — also when only one is selected — that stays visible while its rows scroll), with shortened and suffixed names marked and up to 1000 rows. A red note says the operation cannot be cancelled; Enter there means Cancel. While files are moved the window cannot be closed.

Names are checked before anything moves: a name may have at most 255 characters and at most 255 UTF-8 bytes — the limit of the Linux file system behind a NAS share, where a longer name fails with ENOENT and a Cyrillic letter takes two bytes — and the full path at most 259 characters (so Explorer and players can still open the file). A name that does not fit is shortened with `…`, keeping the extension (only a short tail without spaces counts as one: `1. Introduction` has none). A joined name is cut at the file's own name first (down to 40 characters), then at the folder names; a folder is cut the same way for every file inside it, so its files keep a common head. If even that is impossible because the folder path itself is too long, the dialog shows which files are affected and Continue stays disabled — it says so when turning the hierarchy off would help. The move is all-or-nothing: if any file cannot be moved or renamed (open in another program, access denied, a file with that name appeared), every file moved so far is put back, root files get their old names back, empty subfolders stay, and the error dialog says the folders are unchanged. A file that cannot be put back is listed with its current name in the root, and the list also goes to the log. Existing files are never overwritten: the target name is checked right before each move, and anything there — even a broken link — counts as taken. The run carries out exactly the plan shown in the preview for the chosen options: if the folder changed while the dialog was open, nothing is moved and the dialog asks to run the command again.
...
npm run test:coverage             # same with coverage; flatten-folder, explorer-compare.js and src/main/shell-menu must stay at 100%
...
src/main/utils/explorer-compare.js  Explorer name order: StrCmpLogicalW through koffi, ICU fallback
...
3. **Optional hooks** (all receive `ctx` and the preflight result): `buildFormMessage`, `buildFormSummary` (window mode); `isEmpty`, `buildNothingToDoBody`, `buildBlockedBody`, `buildConfirmMessage` (dialog mode); `buildScanningLabel`, `buildRunningLabel`, `buildErrorBody`. A hook that throws falls back to the default text. Body hooks return text or `{ message, detail, facts, table, tableTitle }`; `facts` (`{ label, value, tone? }`, `tone: 'warn'` for red) replace the free-text summary in the confirm dialog, and `tableTitle` renames its Preview block. `buildConfirmMessage(ctx, pre, options)` may also return `options` — checkboxes `{ name, label, checked, disabled?, nested? }`; changing one calls the hook again with the values as they are now, so the preview can follow them — plus `canContinue: false` to disable Continue and `runOptions`, which `run()` receives as `options`.
