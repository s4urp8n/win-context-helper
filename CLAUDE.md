...
**2026-09-17 update — one Flatten command with options (supersedes parts of 09-16):** `flatten-folder-keep-order` is gone; `flatten-folder` has two confirm-dialog checkboxes, `keepHierarchy` (folder path in the name) and `foldersFirst` (only with the hierarchy; disabled otherwise), both on by default. Preflight scans once (`scanFolder`) and plans three variants (`tree`, `mixed`, `flat` → `pre.variants[key]`, each with its own `planId`); `buildConfirmMessage(ctx, pre, options)` picks one and returns `options` (checkbox descriptors), `canContinue` and `runOptions`. dialog-runner loops on `{ action: 'options', options }` from the shell, rebuilding the confirm body without rescanning, and passes the body's `runOptions` to run (fallback: `pre.planId`). A variant whose names do not fit is shown inside the confirm with Continue disabled; flatten no longer uses `buildBlockedBody` (the hook stays in the runner). With the hierarchy, `keepsOrder` checks that the new names, sorted like Explorer, list the files in the original order (files of one folder with untouched names are not compared); if not, every file of that folder **including root files** gets `NNN - ` in the original order. Names are compared with the real `StrCmpLogicalW` via koffi (`src/main/utils/explorer-compare.js`, ICU fallback) — the old Intl.Collator emulation disagreed with Windows on 276 of 36 981 real sibling pairs (e.g. `X.mp4` / `X_en.vtt`), so the ellipsis guard was dropped. Every table row list starts with a folder header, even for one folder (`buildTable` always groups); the renderer draws the column titles and each group as separate tables so a group title (bold, sticky) leaves with its rows. The confirm state shows titled blocks (Options / Summary / Preview or `tableTitle`); flatten returns its summary as `facts: [{ label, value, tone? }]` instead of `detail`, `asBody` passes `facts` and `tableTitle`, and the "cannot be cancelled" note is white on red. Names must also fit 255 UTF-8 bytes (a Samba/NAS share on ext4/btrfs rejects longer names with ENOENT — verified on Z:); `fitNames` in plan.js shrinks the budget of a file whose fitted name is still too many bytes and refits. Coverage 100% also for `explorer-compare.js`.
...
| `npm run test:coverage` | Vitest with coverage; `plugins/flatten-folder`, `src/main/utils/explorer-compare.js` and `src/main/shell-menu` must stay at 100%. |
